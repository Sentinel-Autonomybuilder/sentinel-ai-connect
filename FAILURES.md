...
| 19 | **Never `taskkill /F /IM node.exe`** -- kills ALL Node.js processes on the machine | dependencies | Kills development environment |
...
| D4 | Windows `taskkill /F /IM node.exe` kills all Node.js processes | Development environment dies during debugging | `taskkill /F /IM node.exe` kills ALL Node.js processes on the machine, including your dev server and any other Node.js tools | Kill only by specific PID: `taskkill /F /PID <pid>` | NEVER use `/IM node.exe` -- always use `/PID` |
...
**Files:** `handshake-RETROSPECTIVE.md`, `handshake-STANDARDS.md`, `handshake-CONVENTIONS.md`, `handshake-MANIFESTO.md`, `handshake-AI-NODE-TEST-INTEGRATION.md`
...
**Files:** `node-tester-HANDOFF.md`, `node-tester-CONVENTIONS.md`, all 12 `node-tester-suggestion-*.md` files
