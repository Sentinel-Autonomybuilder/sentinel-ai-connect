...
export async function checkVpnIp(socksPort) {
  try {
    if (socksPort) {
      ...
      const { checkVpnIpViaSocks } = await import('blue-js-sdk');
      if (typeof checkVpnIpViaSocks === 'function') {
        return await checkVpnIpViaSocks(socksPort, IP_CHECK_TIMEOUT);
      }
      // Fallback: use our own axios + socks-proxy-agent deps
      const axios = (await import('axios')).default;
    ...
    if (err?.code === 'ERR_MODULE_NOT_FOUND') {
      console.warn('[blue-agent-connect] IP check skipped: missing dependency —', err.message?.split("'")[1] || 'unknown');
    }
    return null;
  }
}
