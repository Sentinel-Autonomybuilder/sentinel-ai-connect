#!/usr/bin/env node
/**
 * Agent Connect — Auto-Setup Script
 *
 * Runs on postinstall or manually via: node setup.js
 *
 * Checks:
 * 1. Node.js version >= 20
 * 2. V2Ray binary (local bin/, blue-js-sdk bin/, system PATH)
 * 3. WireGuard installation (optional)
 * 4. .env with MNEMONIC
 *
 * Delegates binary downloads to the blue-js-sdk's setup.js when needed.
 */
...
/**
 * Resolve the blue-js-sdk package directory.
 * Returns the SDK root path or null if not found.
 */
function resolveSdkPath() {
  try {
    const require = createRequire(import.meta.url);
    const sdkEntry = require.resolve('blue-js-sdk');
    return path.dirname(sdkEntry);
  } catch {
    return null;
  }
}
...
function findV2Ray() {
  ...
  // blue-js-sdk bin/ (resolved via npm)
  if (SDK_PATH) {
    searchPaths.push(path.join(SDK_PATH, 'bin', binary));
  }
...
// ─── V2Ray Setup via blue-js-sdk ────────────────────────────────────────────

async function setupV2Ray() {
  if (!SDK_PATH) {
    status.v2ray = { ok: false, detail: 'not found — blue-js-sdk setup.js not available' };
    return;
  }

  const parentSetup = path.join(SDK_PATH, 'setup.js');
  if (!existsSync(parentSetup)) {
    status.v2ray = { ok: false, detail: 'not found — blue-js-sdk setup.js missing' };
    return;
  }

  console.log('[setup] Downloading V2Ray via blue-js-sdk...');
  try {
...
async function checkWireGuard() {
  ...
  // Delegate to blue-js-sdk's setup.js which has full auto-install logic
  if (SDK_PATH) {
    const parentSetup = path.join(SDK_PATH, 'setup.js');
    if (existsSync(parentSetup)) {
...
function printReport() {
  ...
  console.log('');
  console.log('Agent Connect — Setup');
  ...
  if (!status.wallet.ok) {
    ...
    steps.push('Or run: npx blue-agent-connect wallet create');
  }
  ...
  if (steps.length > 0) {
    ...
    console.log('  Ready! Run: npx blue-agent-connect connect');
    console.log('');
  }
}
...
async function main() {
  ...
  if (!status.node.ok) {
    console.error(`[blue-agent-connect] Node.js >= 20 required (found ${process.versions.node})`);
    printReport();
  ...
  // If not found, try blue-js-sdk setup
  if (!v2rayPath) {
    await setupV2Ray();
  }
...
main().catch((err) => {
  console.error(`[blue-agent-connect] Setup error: ${err.message}`);
